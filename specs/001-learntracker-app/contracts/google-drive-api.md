...
await database().ref(`users/${userId}/videos`).push({
...
const deleteAllUserMediaFiles = async (userId: string): Promise<void> => {
  ...
  const videosSnapshot = await database().ref(`users/${userId}/videos`).once('value');
  const voiceNotesSnapshot = await database().ref(`users/${userId}/voice_notes`).once('value');
