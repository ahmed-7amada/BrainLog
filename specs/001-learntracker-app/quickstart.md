# QuickStart Guide: LearnTracker Development Setup
...
cd BrainLog
git checkout 001-learntracker-app
```
...
2. Click "Add project"
3. Enter project name: `learntracker-dev` (or your preferred name)
...
2. Enter iOS bundle ID: `com.learntracker.app` (or your bundle ID)
3. Download `GoogleService-Info.plist`
4. Move file to `ios/LearnTracker/` directory
5. Open Xcode, add `GoogleService-Info.plist` to project (right-click → Add Files)

**Android App**:
1. In Firebase Console, click "Add app" → Android
2. Enter Android package name: `com.learntracker.app` (or your package name)
...
4. Name: "LearnTracker Web Client"
...
3. Name: "LearnTracker iOS"
4. Bundle ID: Same as Firebase iOS app (e.g., `com.learntracker.app`)
5. Copy "Client ID" → This is your `GOOGLE_IOS_CLIENT_ID` in `.env`

**Android Client**:
1. Click "Create Credentials" → "OAuth 2.0 Client ID"
2. Application type: "Android"
3. Name: "LearnTracker Android"
4. Package name: Same as Firebase Android app (e.g., `com.learntracker.app`)
...
**iOS** (`ios/LearnTracker/AppDelegate.mm`):
```objc
...
**iOS** (`ios/LearnTracker/Info.plist`):
```xml
...
module.exports = {
  ...
  apps: {
    'ios.debug': {
      type: 'ios.app',
      binaryPath: 'ios/build/Build/Products/Debug-iphonesimulator/LearnTracker.app',
      build: 'xcodebuild -workspace ios/LearnTracker.xcworkspace -scheme LearnTracker -configuration Debug -sdk iphonesimulator -derivedDataPath ios/build',
    },
    'android.debug': {
      type: 'android.apk',
